...
class Utils {
	...
	static readAnalyticsFile(file, loading, setLoading, selectedFeatures) {
		return new Promise((resolve) => {
			...
			for (let eventName of selectedFeatures)
	...
	static generateRandomData() {
		...
		const bots = new Array(randomNum10to20).fill(0).map(() => {
			return {
				...
				avatar: randomNum10to20_avatars[
		...
		for (
			...
			topDMs.push({
				...
				messageCount:
					Math.floor(Math.random() * (30000 - 1000 + 1)) + 1000,
				...
				oldestMessages: [
					{
						sentence:
							"This Feature is only available when uploading a package",
		...
		for (
			let i = 0;
			i < Math.floor(Math.random() * (10 - 20 + 1)) + 20;
			i++
		) {
			const randomG =
				Math.random() > 0.5
		...
		for (
			let i = 0;
			i < Math.floor(Math.random() * (100 - 10 + 1)) + 10;
			i++
		) {
		...
		return {
			...
			messages: {
				...
				topGuilds: guilds.sort(
					(a, b) => b.messageCount - a.messageCount,
				),
				topGroupDMs,
				characterCount: characterCount,
				messageCount: Math.floor(
					characterCount / (Math.random() * 3) + 1,
				),
				hoursValues,
				favoriteWords: randomWords({ min: 100, max: 700 })
					.map((s) => {
	...
	static isCheckedStats(selectedFeatures, item) {
		return selectedFeatures?.statistics?.includes(
			Object.keys(Events.events).find(
	...
	static async validateOptions(obj, data_d) {
		try {
			...
			if (
				...
				JSON.stringify(Object.keys(data_d))
			)
				return false;
