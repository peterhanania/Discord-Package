	...
	const [emojiType, setEmojiType] = useState<String | null>(
		data?.messages?.topEmojis && data?.messages?.topEmojis.length
			...
			: data?.settings?.recentEmojis &&
			  data?.settings?.recentEmojis.length
			? "recentEmojis"
			: null,
	);
			...
			<div className="lg:flex sm:flex md:flex items-center lg:mx-10 md:mx-8 mx-2 lg:mt-4 md:mt-4 mt-2">
				{!data.dataFile ? (
						...
						content={
							<>
								...
								<p className="text-white text-lg ">
									Exporting is used to share your data with
									others, select the options you want to show,
									click the button to download the JSON file,
									and share the file with your friend. When
									reached, your friend can visit our website
									and directly upload the file they downloaded
									to view your data!
								</p>
								<div className="flex items-center">
									...
									<b className="text-red-400 text-lg pt-1 ">
										Do not share your data with people you
										don&apos;t trust. We are not responsible
										for any data you share.
									</b>
								</div>
							</>
						}
					...
					<span className="dark:text-white text-gray-900">
						You are viewing <b>{data?.user?.username}</b>&apos;s
						data
					</span>
				)}
				{data?.fakeInfo ? (
							...
							<Tippy
								content={
									<>
										...
										<p className="text-white text-lg ">
											Regenerating your data will generate
											a new set of data, allowing you to
											see a demonstration without using
											your data but fake data.
										</p>
										<div className="flex items-center">
											<svg
											...
											<b className="text-red-400 text-lg pt-1 ">
												The data you see is totally fake
												and generated using our
						...
						<div>
							<div className="lg:flex md:flex items-center text-gray-900 dark:text-white">
								...
								<div className="flex items-center">
									...
									data?.messages?.oldestMessages?.length >
										0 ? (
										<Tippy
											content={
												"Your 1000 Oldest Messages"
											}
											animation="scale"
											className="shadow-xl"
										>
									...
									Object.keys(data?.messages?.mentionCount)
										?.length > 0 ? (
										<Tippy
											content={
												Object.values(
					...
					<div className="lg:flex items-center justify-center">
						...
						{!emojiType ? (
							<span className="text-gray-900 dark:text-white text-lg font-bold w-full">
								No Emojis Found or{" "}
								{data?.dataFile ? "they " : "you "}disabled all
								emoji options
							</span>
						) : (
							""
					...
					data?.guilds ? (
						<ul className="text-gray-900 dark:text-white lg:text-xl md:text-xl font-bold mt-2 ">
							{data?.settings?.appearance?.theme ? (
								<li className="flex items-center">
									...
									{data?.dataFile ? "They " : "You "}prefer
									Discord{" "}
									{data.settings.appearance.theme.toLowerCase()}{" "}
									mode
								</li>
							) : (
								""
							...
							{data?.settings?.appearance?.developerMode ? (
								<li className="flex items-center">
									...
									{data?.dataFile ? "They " : "You "}are using
						...
						<div className="justify-items-center">
							<div className="text-gray-900 dark:text-white text-xl font-bold ">
								{data?.dataFile ? "They " : "You "}disabled
								these Options
							</div>
						</div>
					)}
							...
							content={
								<>
									...
									<p className="text-white text-lg ">
										...
										{data?.dataFile ? "they " : "you "} are
										most active on Discord sending messages.
									</p>
									<div className="flex items-center">
										<svg
										...
										<b className="text-red-400 text-lg pt-1 ">
											...
											{data?.dataFile
												? "their "
												: "your "}{" "}
											device timezone. If the graph is
											inacurrate, make sure{" "}
											{data?.dataFile
												? "their "
												: "your "}{" "}
											device timezone is the same timezone
											as the usual timezone you use when
						...
						<>
							...
							<div className="flex grow rounded-sm overflow-y-auto overflow-x-hidden h-[700px]">
								<div className="flex flex-col w-full px-3 pb-4 lg:px-3 md:px-3 lg:pt-0 md:pt-0 pt-2">
									...
									{!data?.messages?.topChannels ? (
										<div className="px-10 text-gray-900 dark:text-white text-3xl font-bold flex flex-col justify-center content-center align-center w-full h-full">
											No Data was found or this option is
											disabled
										</div>
									) : (
										""
			...
			<div className="lg:grid my-4 grid-rows-2 grid-flow-col gap-4 lg:mx-10 md:mx-8 mx-2 lg:mt-4 md:mt-4 mt-2">
				<div className="lg:px-4 md:px-4 px-1 py-2 bg-gray-300 dark:bg-[#2b2d31] animate__fadeIn animate__delay-1s rounded-lg row-span-3 lg:flex md:flex items-center justify-center">
					<div className="lg:mr-14 lg:ml-8 md:mr-12 md:ml-7 mb-2 lg:mb-0 md:mb-0">
						...
						>
							{data?.dataFile ? "Their" : "Your"} <br /> Discord{" "}
							<br /> Bots
						</span>
				...
				<div className="px-4 py-2 lg:my-0 my-4 bg-gray-300 dark:bg-[#2b2d31] animate__fadeIn animate__delay-1s rounded-lg row-span-3 col-span-2 ">
					...
					{!data?.payments ? (
						<div className="flex items-center gap-2">
							{" "}
							<span className="text-gray-900 dark:text-white font-bold">
								{data?.dataFile ? "They " : "You "}have no
								payments or this option is disabled by{" "}
								{data?.dataFile ? "them" : "you"}.
							</span>
						</div>
					) : (
						<>
								...
								<span className="text-gray-900 dark:text-white text-xl flex items-center">
									...
									<p className="mx-1 font-extrabold text-blue-500 inline-flex">
										<CountUp
											...
											delay={2}
										/>
										{data?.payments?.transactions?.length >
										0 ? (
											<div
							...
							<div className="flex items-center gap-2">
								...
								<ul className="text-gray-900 dark:text-white lg:text-xl md:text-xl font-bold list-disc mt-2 ml-6 text-xs">
									{data?.payments?.transactions?.map(
										(t: any, i: number) => {
											return (
												<li
													key={i}
								...
								<Tippy
									content={
										<>
											...
											<p className="text-white text-lg ">
												Gifted Nitro is the Nitro Gifted
												to{" "}
												{data?.dataFile
													? "them "
													: "you "}{" "}
			...
			<div className="gap-4 lg:mx-10 md:mx-8 mx-2 lg:mt-4 md:mt-4 mt-2 pb-10">
				<div className="lg:px-4 md:px-4 px-1 py-2 bg-gray-300 dark:bg-[#2b2d31] animate__fadeIn animate__delay-1s rounded-lg text-left w-full">
						...
						<Tippy
							content={
								<>
									...
									<p className="text-white text-lg ">
										Statistics are analytic data used by
										Discord for a better user experience.
									</p>
					...
					<div className="text-gray-900 dark:text-white md:text-xl lg:text-xl font-bold text-[16px] mt-2">
						{data?.statistics
							? Object.keys(data?.statistics)?.map(
									(t: any, i: number) => {
										if (!t) return;
										if (
											(EventsJSON.events as any)[t] &&
											data?.statistics[t]
										) {
											return (
												<div key={i}>
					...
					<div className="bg-gray-300 dark:bg-[#232323] px-6 py-2">
						...
						>
							...
							<p className="text-gray-900 dark:text-white text-sm max-w-[200px] text-left">
								Enjoying what you see? Star this repository on
								Github and follow me! It means a lot 😊!
							</p>
						</a>
					</div>
				</div>
