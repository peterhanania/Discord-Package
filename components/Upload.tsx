      ...
      // eslint-disable-next-line 
			async function startUpload() {
				...
				if (!validPackage) {
					setLoading(null);
					setError(
						"This package is not a valid package. Please try again.",
					);
					return;
				}
				...
				async function extractData(files: any, options: any) {
					...
					if (isDebug) {
						...
						setLoading(
							"Loading User Information|||Loading Main Information",
						);
					} else await delay(100);
					...
					if (isDebug) {
						...
						await delay(700);
						setLoading(
							"Loading User Information|||Loading Setting Frecency",
						);
					} else await delay(100);
					setPercent(13);
					...
					if (options.connections) {
						if (isDebug) {
							setLoading(
								"Loading User Information|||Loading User Connections",
							);
							await delay(600);
						} else {
						...
						if (
							...
							if (
								...
								const cncs = Object.values(
									userInformationData.connections,
								)
									.filter(
										(s: any): any => s.type !== "contacts",
									)
									.map((e: any): any => {
										return {
											type: e.type,
								...
								if (isDebug)
									console.log(
										chalk.bold.blue(`[DEBUG] `) +
											...
											`  ${chalk.yellow(
												`Loaded ${cncs.length} connections`,
											)}`,
									);
							}
						}
					...
					if (isDebug) {
						setLoading(
							"Loading User Information|||Loading User Payments",
						);
						await delay(700);
					} else {
					...
					const confirmedPayments =
						userInformationData?.payments.filter(
							...
							if (isDebug)
								console.log(
									chalk.bold.blue(`[DEBUG] `) +
										...
										`  ${chalk.yellow(
											`Loaded ${trns.length} transactions`,
										)}`,
								);
						}
					}
					...
					if (isDebug) {
						...
						setLoading(
							"Loading User Information|||Loaded User Information",
						);
						await delay(2000);
					} else await delay(100);
					setPercent(25);
					...
					if (isDebug)
						console.log(
							chalk.bold.blue(`[DEBUG] `) +
								...
								`  ${chalk.yellow(
									`Loaded ${messagesRead} message files`,
								)}`,
						);
						...
						if (isDebug) {
							console.log(
								chalk.bold.blue(`[DEBUG] `) +
									...
									`  ${chalk.yellow(
										`Calculating top channels`,
									)}`,
							);
							setLoading(
								"Loading Messages|||Calculating top Channels",
							);
						} else await delay(100);
						...
						data.messages.topChannels = channels
							...
							.map((channel) => {
								...
								const topEmojisANDcustom = channel.messages
									.map((message: any): any => {
										const emojis = Utils.getEmojiCount(
											message.words,
										);
										const customEmojis =
											Utils.getCustomEmojiCount(
												message.words,
											);
								...
								return {
									...
									topEmojis: options.other.topEmojis
										...
										: null,
									topCustomEmojis: options.other
										.topCustomEmojis
										? topCustomEmojis.sort(
												(a, b) => b.count - a.count,
										  )
										: null,
								};
					...
					if (options.messages.topDMs) {
						...
						if (isDebug) {
							...
							setLoading(
								"Loading Messages|||Calculating top DMs",
							);
						} else await delay(100);
					...
					if (options.messages.topGuilds) {
						...
						if (isDebug) {
							...
							setLoading(
								"Loading Messages|||Calculating top Guilds",
							);
						} else await delay(100);
						...
						if (isDebug) {
							console.log(
								chalk.bold.blue(`[DEBUG] `) +
									...
									`  ${chalk.yellow(
										`Loading top group DMs`,
									)}`,
							);
							setLoading(
								"Loading Messages|||Calculating top Group DMs",
							);
						} else await delay(100);
					...
					if (options.messages.characterCount) {
						...
						if (isDebug) {
							...
							setLoading(
								"Loading Messages|||Getting your character Count",
							);
							await delay(700);
						} else await delay(100);
					...
					if (options.messages.oldestMessages) {
						...
						if (isDebug) {
							...
							setLoading(
								"Loading Messages|||Getting your oldest messages",
							);
							await delay(700);
						} else await delay(100);
						...
						data.messages.oldestMessages = oldestInTotal.slice(
							0,
							1000,
						);
						if (isDebug)
					...
					if (options.messages.attachmentCount) {
						if (isDebug) {
							setLoading(
								"Loading Messages|||Getting your attachment Count",
							);
							console.log(
								chalk.bold.blue(`[DEBUG] `) +
									chalk.bold.cyan(
							...
							.map((channel) => {
								const words = channel.messages
									.map((message: any) => {
										...
										const attachmentsCleaned =
											attachments.map(
												(attachment: any) => {
													const mtch = regex.test(
														attachment,
													)
														? attachment.match(
																regex,
					...
					if (options.messages.mentionCount) {
						if (isDebug) {
							console.log(
								chalk.bold.blue(`[DEBUG] `) +
									...
									`  ${chalk.yellow(
										`Loading your mention Count`,
									)}`,
							);
							setLoading(
								"Loading Messages|||Getting your mention Count",
							);
						} else await delay(100);
						...
						const oldestInChannel = channels
							.map((channel) => {
								const words = channel.messages
									.map((message: any) => {
										const discordChannelMentionRegex =
											/<#[0-9]*>/gi;
										const discordUserMentionRegex =
											/<@[0-9]*>/gi;
										const discordRoleMentionRegex =
											/<@&[0-9]*>/gi;
											...
											() => {
												const o = {
													channel:
														message.words.filter(
															(word: any) =>
																discordChannelMentionRegex.test(
																	word,
																),
														).length,
													user: message.words.filter(
														(word: any) =>
															discordUserMentionRegex.test(
																word,
															),
													).length,
													role: message.words.filter(
														(word: any) =>
															discordRoleMentionRegex.test(
																word,
															),
													).length,
													here: message.words.filter(
														(word: any) =>
															/@here/g.test(word),
													).length,
													everyone:
						...
						if (isDebug) {
							console.log(
								chalk.bold.blue(`[DEBUG] `) +
									...
									`  ${chalk.yellow(
										`Calculating your top emojis`,
									)}`,
							);
							setLoading(
								"Loading Messages|||Getting your top emojis",
							);
						} else await delay(100);
						...
						if (isDebug) {
							console.log(
								chalk.bold.blue(`[DEBUG] `) +
									...
									`  ${chalk.yellow(
										`Calculating your top custom emojis`,
									)}`,
							);
							setLoading(
								"Loading Messages|||Getting your top custom emojis",
							);
						} else await delay(100);
						...
						if (isDebug)
							console.log(
								chalk.bold.blue(`[DEBUG] `) +
									...
									`  ${chalk.yellow(
										`Calculating your top hours on discord`,
									)}`,
							);
						for (let i = 0; i < 24; i++) {
						...
						if (isDebug)
							console.log(
								chalk.bold.blue(`[DEBUG] `) +
									...
									`  ${chalk.yellow(
										`Calculated your top hours on discord`,
									)}`,
							);
					}
					...
					if (options.other.favoriteWords) {
						if (isDebug) {
							setLoading(
								"Loading Messages|||Calculating your favorite words",
							);
							await delay(600);
						} else await delay(100);
						setPercent(74);
					...
					if (options.other.showCurseWords) {
						if (isDebug) {
							setLoading(
								"Loading Messages|||Calculating your curse words",
							);
							await delay(700);
						} else await delay(100);
						setPercent(76);
					...
					if (options.other.showLinks) {
						...
						if (isDebug) {
							setLoading(
								"Loading Messages|||Calculating your general links",
							);
							await delay(600);
						} else await delay(100);
					...
					if (options.other.showDiscordLinks) {
						...
						if (isDebug) {
							setLoading(
								"Loading Messages|||Calculating your discord links",
							);
							await delay(700);
						} else await delay(100);
							...
							if (isDebug)
								console.log(
									chalk.bold.blue(`[DEBUG] `) +
										...
										`  ${chalk.yellow(
											`Loaded ${botsArr.length} bots`,
										)}`,
								);
						} else {
							if (isDebug) {
								setLoading(
									"Loading User Bots|||Bots not Found",
								);
								await delay(700);
							}
						}
					...
					if (userInformationData.flags && options.user.badges) {
						...
						const badges = BitField.calculate(
							userInformationData.flags,
						);
						...
						if (
							data?.bots?.filter((bot: any) => bot.verified)
								?.length > 0 &&
				...
				extractData(files, selectedFeatures)
					.then(async (data) => {
						...
						setLoading(
							"Rendering Data|||✨ Spicing up things for you ✨",
						);
						if (data.disabledDebugMode) {
							toast(
								...
								>
									...
									<span className="text-black dark:text-white">
										Heya user, your package took more than 1
										minute to load. Therefore you might be
										experiencing a slow loading of the file
										using debug mode. Therefore, I have
										disabled debug mode for you to speed up
										the loading of your package! To
										re-enable it, go on the home page,
										select settings and then check the
							...
							toast(
								...
								>
									...
									<br />
									<span className="text-black dark:text-white">
										We are currently Rendering your Data you
										might experience a bit of lag. If you
										run through severe issues join our
					...
					.catch((err) => {
						...
						if (
							err.message === "invalid_package_missing_messages"
						) {
							setLoading(null);
							setError(
				...
				readFile_.onload = function (e: any) {
					...
					if (data) {
						if (data.dataFile) {
							...
							setLoading(null);
							setError(
								"The JSON file you have uploaded is not valid",
							);
						}
					} else {
						setLoading(null);
						setError(
							"The JSON file you have uploaded is corrupted",
						);
					}
				};
								...
								>
									<div className="flex flex-col justify-center items-center pt-5 pb-6">
										...
										<p className="text-xs text-gray-500 dark:text-gray-400">
											Upload your discord package or JSON
											here
										</p>
